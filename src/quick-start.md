...
创建 KISSY 沙箱时你需要指定要载入的模块，KISSY 的功能是模块化的，包括`node`，`event`，`ajax`（在5.0版本推荐用`io`）等。这个例子中载入了`node`模块，在沙箱中可以通过回调参数来使用`node`的 API。**特别的**，`node`模块中最常用的 API 被挂载在 KISSY 对象上，作为快捷调用方式。比如`S.all == Node.all`和`S.one == Node.one`。
...
在 5.0 中，Ajax 功能被命名为 io 模块，使用方法：
...
gallery 鼓励分享，任何人都可以为 gallery 提交自己的组件。在 KISSY 5.0 中内置了 gallery package，使用 gallery 组件非常方便：
