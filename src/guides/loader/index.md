...
> loader 是脚本加载器的实现， 弥补了 javascript 语言机制的不足, 提供类似其他语言原生的模块化机制。KISSY loader 实现了 [KISSY 模块规范]({{{version}}}/guides/loader/kmd.html)。
...
本地开发时，模块名称可留空，不过在部署阶段需要使用[Kissy Mobule Compiler](https://github.com/daxingplay/kmc) 生成模块依赖关系表，或者直接生成合并后的文件。
...
<h3 id="config">config()  <code>&lt;static&gt;</code></h3>
...
> 模块名也可以省略, 不过部署阶段需要使用 [KISSY Module Compiler](https://github.com/daxingplay/kmc) 静态打包。
...
载入并运行模块,和`add`一起使用，详细用法参照[KISSY 模块规范]({{{version}}}/guides/loader/kmd.html)，`callback`类型可以是function也可以是object。参数说明：
